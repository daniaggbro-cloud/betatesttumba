local loadstring = function(...)
	...
	if err and tumbahub then tumbahub:CreateNotification('TumbaHub', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/zxcbest957-pixel/TumbaV6/'..readfile('tumbascript/profiles/commit.txt')..'/'..select(1, path:gsub('tumbascript/', '')), true) end)
...
local vm = loadstring(downloadFile('tumbascript/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('TumbaHub', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
