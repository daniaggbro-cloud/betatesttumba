...
local tween = tumbahub.Libraries.tween
local targetinfo = tumbahub.Libraries.targetinfo
local getfontsize = tumbahub.Libraries.getfontsize
local getcustomasset = tumbahub.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('tumbascript/assets/new/blur.png')
...
local function canClick()
	...
	return (not tumbahub.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function serverHop(pointer, filter)
	...
	if data and data.data then
		...
		notif('TumbaHub', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

tumbahub:Clean(lplr.OnTeleport:Connect(function()
...
tumbahub.Libraries.string = loadstring(downloadFile('tumbascript/libraries/string.lua'), 'string')()
...
local hash = loadstring(downloadFile('tumbascript/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('tumbascript/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('tumbascript/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
tumbahub.Libraries.entity = entitylib
tumbahub.Libraries.whitelist = whitelist
tumbahub.Libraries.prediction = prediction
tumbahub.Libraries.hash = hash
tumbahub.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if tumbahub.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and tumbahub.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(tumbahub.Categories.Friends.Options['Friends color'].Hue, tumbahub.Categories.Friends.Options['Friends color'].Sat, tumbahub.Categories.Friends.Options['Friends color'].Value)
		end
	...
	tumbahub:Clean(tumbahub.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	tumbahub:Clean(tumbahub.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	tumbahub:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	tumbahub:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = tumbahub.Uninject
				tumbahub.Uninject = function()
					notif('TumbaHub', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('TumbaHub', plr.Name..' is using tumbahub!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				tumbahub:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				tumbahub:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('tumbascript/profiles/whitelist.json') and readfile('tumbascript/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				tumbahub:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and tumbahub.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement and whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'TUMBAHUB ANNOUNCEMENT: '..whitelist.data.Announcement.text
				...
				pcall(function()
					writefile('tumbascript/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.KillTumbaHub then
				tumbahub:Uninject()
				return true
			end
		end
	...
	whitelist.commands = {
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = tumbahub.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
		...
		toggle = function(args)
			...
			if args[1]:lower() == 'all' then
				for i, v in tumbahub.Modules do
...
run(function()
	...
	AimAssist = tumbahub.Categories.Combat:CreateModule({
		Name = 'Aim Assist',
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not tumbahub.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	AutoClicker = tumbahub.Categories.Combat:CreateModule({
		Name = 'Auto Clicker',
		Function = function(callback)
			if callback then
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not tumbahub.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = tumbahub.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TriggerBot = tumbahub.Categories.Combat:CreateModule({
		Name = 'Trigger Bot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AntiFall = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Anti Fall',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Fly = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			if Platform then
...
run(function()
	...
	HighJump = tumbahub.Categories.Blatant:CreateModule({
		Name = 'High Jump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitBoxes = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Hit Boxes',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Invisible = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Invisible',
...
run(function()
	...
	Killaura = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	LongJump = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Long Jump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MouseTP = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Mouse TP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Phase = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speed = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	Spider = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SpinBot = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Spin Bot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Target Strafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = tumbahub.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = tumbahub.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('tumbascript/assets/new/arrowmodule.png')
	...
	Arrows = tumbahub.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(tumbahub.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = tumbahub.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(tumbahub.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	ESP = tumbahub.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(tumbahub.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = tumbahub.Categories.Render:CreateModule({
		Name = 'Gaming Chair',
		Function = function(callback)
			if callback then
				if tumbahub.ThreadFix then
				...
				--movingsound.SoundId = downloadTumbaHubAsset('tumbahub/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadTumbaHubAsset('tumbahub/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = tumbahub.Modules.Fly and tumbahub.Modules.Fly.Enabled or tumbahub.Modules.LongJump and tumbahub.Modules.LongJump.Enabled or tumbahub.Modules.InfiniteFly and tumbahub.Modules.InfiniteFly.Enabled
...
run(function()
	...
	Health = tumbahub.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = tumbahub.Categories.Render:CreateModule({
		Name = 'Name Tags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(tumbahub.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	PlayerModel = tumbahub.Categories.Render:CreateModule({
		Name = 'Player Model',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	Radar = tumbahub:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('tumbascript/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(tumbahub.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = tumbahub.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SessionInfo = tumbahub:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.tumbahubsessioninfo = '"..httpService:JSONEncode(tumbahub.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.tumbahubsessioninfo then
					for i, v in httpService:JSONDecode(shared.tumbahubsessioninfo) do
						if tumbahub.Libraries.sessioninfo.Objects[i] and v.Saved then
							tumbahub.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if tumbahub.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
	
						for i, v in tumbahub.Libraries.sessioninfo.Objects do
	...
	tumbahub:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if tumbahub.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (tumbahub.gui.AbsoluteSize.X / 2)
	...
	addBlur(infoholder)
	tumbahub.Libraries.sessioninfo = {
		Objects = {},
	...
	tumbahub.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = tumbahub.gui.AbsoluteSize
	...
	Tracers = tumbahub.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(tumbahub.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = tumbahub.gui
	
	Waypoints = tumbahub.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = tumbahub.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = tumbahub.Categories.Utility:CreateModule({
		Name = 'Anti Ragdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = tumbahub.Categories.Utility:CreateModule({
		Name = 'Auto Rejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = tumbahub.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = tumbahub.Categories.Utility:CreateModule({
		Name = 'Chat Spammer',
		Function = function(callback)
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	Disabler = tumbahub.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	tumbahub.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				for _, v in tumbahub.Modules do
					if v.Enabled then
						v:Toggle()
					end
				end
...
run(function()
	...
	Rejoin = tumbahub.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = tumbahub.Categories.Utility:CreateModule({
		Name = 'Server Hop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.tumbahubserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.tumbahubserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.tumbahubserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				tumbahub.Save = function() end
				if tumbahub.Profile ~= Profile.Value then
					tumbahub.Profile = Profile.Value
					tumbahub:Load(true, Profile.Value)
				end
	...
	StaffDetector = tumbahub.Categories.Utility:CreateModule({
		Name = 'Staff Detector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	tumbahub.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = tumbahub.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = tumbahub.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = tumbahub.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = tumbahub.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = tumbahub.Categories.Minigames:CreateModule({
		Name = 'Murder Mystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = tumbahub.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = tumbahub.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Cape = tumbahub.Legit:CreateModule({
		Name = 'Cape',
		Function = function(callback)
			if callback then
				part = Instance.new('Part')
...
run(function()
	...
	ChinaHat = tumbahub.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if tumbahub.ThreadFix then
...
run(function()
	...
	Clock = tumbahub.Legit:CreateModule({
		Name = 'Clock',
		Category = 'Hud',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Disguise = tumbahub.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FOV = tumbahub.Legit:CreateModule({
		Name = 'FOV',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FPS = tumbahub.Legit:CreateModule({
		Name = 'FPS',
		Category = 'Hud',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Keystrokes = tumbahub.Legit:CreateModule({
		Name = 'Keystrokes',
		Category = 'Hud',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = tumbahub.Legit:CreateModule({
		Name = 'Memory',
		Category = 'Hud',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Ping = tumbahub.Legit:CreateModule({
		Name = 'Ping',
		Category = 'Hud',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SongBeats = tumbahub.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Coords = tumbahub.Legit:CreateModule({
		Name = 'Coords',
		Category = 'Hud',
	...
	for i, v in {'x', 'y', 'z'} do
		...
		shadow.Parent = gui
		tumbahub:Clean(display:GetPropertyChangedSignal('Text'):Connect(function()
...
run(function()
	...
	Speedmeter = tumbahub.Legit:CreateModule({
		Name = 'Speedmeter',
		Category = 'Hud',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TimeChanger = tumbahub.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	InfiniteJump = tumbahub.Categories.Blatant:CreateModule({
		Name = "Infinite Jump",
...
run(function()
	...
	ZoomUnlocker = tumbahub.Categories.Render:CreateModule({
		Name = 'Zoom Unlocker',
...
run(function()
	...
	PromptDuration = tumbahub.Categories.Utility:CreateModule({
...
run(function()
	...
	PromptExtender = tumbahub.Categories.Utility:CreateModule({
...
run(function()
	...
	local function ChangeFFlag(suc)
		...
		if not success or typeof(json) ~= 'table' then
			notif('TumbaHub', 'Invalid json format for fflag', 12, 'warning')
			return
		end
		...
		notif('TumbaHub', 'FFlags applied, Go in a new game to take effect', 12, 'info')
	end

	FFlag = tumbahub.Legit:CreateModule({
		Name = 'FFlag Editor',
		...
		Function = function(call)
			if call then
				...
				notif('TumbaHub', 'Inorder to disable fflags you have applied, You need to restart roblox', 20, 'info')
			end
		end
	})
