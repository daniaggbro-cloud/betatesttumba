...
local tumbahubEvents = setmetatable({}, {
...
local uipallet = tumbahub.Libraries.uipallet
local tween = tumbahub.Libraries.tween
local color = tumbahub.Libraries.color
...
run(function()
	...
	local function updateStore(newStore, oldStore)
		if newStore.GameCurrency ~= oldStore.GameCurrency then
			tumbahubEvents.CurrencyChange:Fire(table.clone(newStore.GameCurrency.Quantities))
		end
		...
		if newStore.Inventory ~= oldStore.Inventory then
			...
			tumbahubEvents.InventoryAmountChanged:Fire()
		end
	...
	tumbahub:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	tumbahub:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
	...
	tumbahub:Clean(function()
		for _, v in tumbahubEvents do
			v:Destroy()
		end
		table.clear(ControllerTable)
		table.clear(RemoteTable)
		table.clear(tumbahubEvents)
		table.clear(skywars)
...
run(function()
	...
	Sprint = tumbahub.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Velocity = tumbahub.Categories.Combat:CreateModule({
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	InvMove = tumbahub.Categories.Blatant:CreateModule({
		Name = 'InvMove',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local anims, armC0 = tumbahub.Libraries.auraanims
		...
		Function = function(callback)
			if callback then
				...
				repeat
					...
					if Attacking and tumbahub.ThreadFix then
						setthreadidentity(8)
					end
...
run(function()
	...
	NoFall = tumbahub.Categories.Blatant:CreateModule({
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	local ProjectileAimbot = tumbahub.Categories.Blatant:CreateModule({
...
run(function()
	...
	ProjectileAura = tumbahub.Categories.Blatant:CreateModule({
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Scaffold = tumbahub.Categories.Utility:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	ChestSteal = tumbahub.Categories.World:CreateModule({
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = tumbahub.Categories.Inventory:CreateModule({
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				AutoBuy:Clean(tumbahubEvents.CurrencyChange.Event:Connect(buyCheck))
...
run(function()
	...
	AutoConsume = tumbahub.Categories.Inventory:CreateModule({
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
				AutoConsume:Clean(tumbahubEvents.InventoryAmountChanged.Event:Connect(consumeCheck))
...
run(function()
			...
			BreakerUI = skywars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						Image = getcustomasset('tumbascript/assets/new/blur.png'),
	...
	Breaker = tumbahub.Categories.Minigames:CreateModule({
		Name = 'Breaker',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Viewmodel = tumbahub.Categories.Legit:CreateModule({
		Name = 'Viewmodel',
		Function = function(callback)
			if callback then 
				ViewmodelMotor = Instance.new('Motor6D')
				tumbahub:Clean(ViewmodelMotor)
				tumbahub:Clean(runService.RenderStepped:Connect(function()
				...
				tumbahub:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
				if entitylib.isAlive then 
