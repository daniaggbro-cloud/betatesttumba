...
local drawingactor = loadstring(downloadFile('tumbascript/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		local oldreload = shared.tumbahubreload
		tumbahub.Load = function()
		...
		task.spawn(function()
			repeat task.wait() until not shared.tumbahub
			local executionString = "loadfile('tumbascript/main.lua')("..drawingactor..")"
			...
			if oldreload then
				executionString = 'shared.tumbahubreload = true\n'..executionString
			end
			...
			notif('TumbaHub', 'Failed to find actor', 10, 'alert')
		end)
	else
		tumbahub.Load = function()
			notif('TumbaHub', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('TumbaHub', 'Failed to hook ('..id..')', 10, 'alert')
	end
...
run(function()
	...
	tumbahub:Clean(Drawing.kill or function() end)
...
run(function()
	...
	GrenadeTP = tumbahub.Categories.Blatant:CreateModule({
		Name = 'GrenadeTP',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	GunModifications = tumbahub.Categories.Blatant:CreateModule({
...
run(function()
	...
	GrenadeESP = tumbahub.Categories.Render:CreateModule({
		Name = 'GrenadeESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NoHurtCam = tumbahub.Categories.Render:CreateModule({
		Name = 'NoHurtCam',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ThirdPerson = tumbahub.Categories.Render:CreateModule({
		Name = 'ThirdPerson',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoRespawn = tumbahub.Categories.Utility:CreateModule({
		Name = 'AutoRespawn',
		Function = function(callback)
			if callback then
...
run(function()
	...
	PickupRange = tumbahub.Categories.Utility:CreateModule({
		Name = 'PickupRange',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	BulletTracers = tumbahub.Categories.Legit:CreateModule({
		Name = 'BulletTracers',
		Function = function(callback)
			if callback then 
